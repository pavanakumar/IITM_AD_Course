...
begin
    ...
    function IsenVortexUV(x, y, Γ, x0, y0, R)
	  dy = (y - y0)
	  dx = (x - x0)
	  d2 = dx * dx + dy * dy
	  R2 = R * R
	  term = exp(-d2 / R2 / 2)
	  return (-Γ / 2π * dy / R2 * term, Γ / 2π * dx / R2 * term)
	end
...
md"""
# Exact solution to Isentropic Vortex Convection

In CFD a popular test case for testing numerical schemes is the solution to the 2D Isentropic vortex. This has an exact solution that can be used for validation of the solver. We assume a uniform free-stream i.e.,
...
md"""
## Velocity contours of Isentropic vortex solution
"""

# ╔═╡ ac76885f-8ccd-436e-b9ae-bb0b88bda945
begin
	num_nodes = 100
	x_plot = range(-1, 1, num_nodes)
	y_plot = range(-1, 1, num_nodes)
	Γ_plot = 1.0
	x0_plot = 0.0
	y0_plot = 0.0
	R_plot = 0.1
...
Vary the value of ϵ in the code to understand how the solution changes.
